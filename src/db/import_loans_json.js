...
async function run() {
  ...
  for (const item of items) {
    ...
    let application_process = [
      'Register on official website or branch',
      'Fill out application form and submit KYC',
    ...
    if (category === 'bank_or_nbfc') {
      ...
      const isVL = name.toLowerCase().includes("vidya lakshmi");
      const isCSIS = name.toLowerCase().includes("central sector interest") || name.toLowerCase().includes("csis");
      
      collateral_requirement = isVL ? 'Depends on selected bank' : 'Often collateral-free up to ₹7.5 Lakhs';
      interest_subsidy = item.benefit || 'Subsidized loan rates';
      highlights = [
        item.benefit || 'Sponsored by Ministry of Education, Government of India',
        'Encourages education accessibility for all economic sections',
        'Easy digital or direct coordination'
      ];
